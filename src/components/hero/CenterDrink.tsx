...
export const CenterDrink: React.FC<CenterDrinkProps> = ({
  ...
  // Subtle organic micro-motion (breathing life into the product, 5.2s sine cycle)
  useEffect(() => {
    ...
    <div className="relative z-30 flex flex-col items-center justify-center pointer-events-none select-none">
      ...
      <div ref={floatingRef} className="relative flex flex-col items-center justify-center">
        ...
        <div className="relative w-[185px] sm:w-[235px] md:w-[275px] lg:w-[305px] aspect-[3/4] flex items-center justify-center">
          {/* 1. Base Physically Accurate 3-Tier Contact Shadow */}
          <div className="absolute -bottom-2 left-1/2 -translate-x-1/2 w-full flex items-center justify-center pointer-events-none z-0">
            ...
            <div className="absolute w-[80%] h-6 bg-[#071309]/85 rounded-full blur-[12px] transform-gpu translate-y-2" />
            {/* Ambient Occlusion Core Contact Line */}
            <div className="absolute w-[60%] h-3 bg-black/95 rounded-full blur-[3.5px] transform-gpu translate-y-1" />
            {/* Ambient Matcha Light Spill / Ground Tint */}
            <div className="absolute w-[48%] h-3.5 bg-[#29482a]/40 rounded-full blur-[7px] transform-gpu" />
          </div>

          {/* 2. Previous Drink Layer (Crossfade out) */}
          {previousFlavor && isTransitioning && (
            <div className="absolute inset-0 w-full h-full flex items-center justify-center z-10 animate-fadeOut">
              <img
                src={previousFlavor.productImage}
                alt={previousFlavor.nameEn}
                className="w-full h-full object-contain pointer-events-none opacity-0 transition-all duration-400 transform scale-[0.985]"
                draggable={false}
              />
            </div>
          )}

          {/* 3. Current Active Drink (Crossfade in) */}
          <div className="relative w-full h-full flex items-center justify-center z-10">
            <img
              key={currentFlavor.id}
              src={currentFlavor.productImage}
              alt={currentFlavor.nameEn}
              className="w-full h-full object-contain pointer-events-none transition-all duration-400 ease-out transform scale-100"
              draggable={false}
            />

            {/* 4. Subtle Environmental Rim-Light & Center Highlight */}
            <div
              className="absolute inset-0 w-full h-full pointer-events-none mix-blend-overlay opacity-25 rounded-3xl"
              style={{
                background:
            ...
            <div
              className="absolute inset-x-0 bottom-0 h-1/4 pointer-events-none rounded-b-3xl opacity-20"
              style={{
                background:
          ...
          <div className="absolute top-[52%] sm:top-[53%] left-1/2 -translate-x-1/2 -translate-y-1/2 w-full text-center pointer-events-none px-2 z-20">
            ...
            <h2
              key={currentFlavor.id}
              className="font-headline text-lg sm:text-2xl md:text-[26px] font-bold text-white tracking-wide drop-shadow-[0_4px_16px_rgba(0,0,0,0.95)] transition-all duration-300 transform-gpu leading-tight"
            >
          ...
          {/* 7. Foreground Occlusion Mist passing directly in front of cup base */}
          <div className="absolute -bottom-2 inset-x-0 h-8 pointer-events-none z-25 flex items-center justify-center">
            <div className="w-[82%] h-full bg-gradient-to-t from-[#122416]/65 via-[#1d351f]/20 to-transparent rounded-full blur-[5px] opacity-70" />
          </div>
