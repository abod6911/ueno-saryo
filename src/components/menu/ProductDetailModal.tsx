...
export const ProductDetailModal: React.FC<ProductDetailModalProps> = ({ item, onClose }) => {
  ...
  const tastingNotes = locale === 'ar' ? item.tastingNotes?.ar : item.tastingNotes?.en;
  const ingredients = locale === 'ar' ? item.ingredients?.ar : item.ingredients?.en;
      ...
      >
        ...
        <div className="relative w-full bg-[#122416] p-6 sm:p-8 flex items-center justify-between text-[#f8f7f1] overflow-hidden">
          ...
          <button
            ...
            className="relative z-10 w-9 h-9 sm:w-10 sm:h-10 rounded-full bg-white/10 hover:bg-white/20 text-white flex items-center justify-center transition-all active:scale-95 border border-white/15"
          >
        ...
        <div className="p-6 sm:p-8 overflow-y-auto custom-scrollbar flex flex-col gap-6 text-[#181813]">
              ...
              <div className="flex flex-wrap gap-2">
                ...
                {item.caffeineLevel && (
                  <div className="flex items-center gap-1.5 px-3 py-1 rounded-full bg-emerald-50 text-emerald-900 text-xs font-semibold border border-emerald-200">
                    ...
                    <span>{t.menu.caffeine}: {item.caffeineLevel}</span>
                  </div>
                )}
                {item.temperature && (
                  <div className="px-3 py-1 rounded-full bg-black/5 text-xs text-[#181813]/70 font-medium">
                    {t.menu.temperature}: {item.temperature}
                  </div>
                )}
              </div>

              <p className="text-xs sm:text-sm text-[#181813]/75 leading-relaxed pt-1">
                {description}
              </p>
            </div>
          </div>
            ...
            <div className="flex flex-col gap-2 pt-4 border-t border-black/5">
              ...
              <div className="flex flex-wrap gap-2">
                {tastingNotes.map((note, idx) => (
                  <span
                    key={idx}
                    className="text-xs bg-[#122416]/5 text-[#122416] px-3 py-1 rounded-full font-medium flex items-center gap-1.5"
                  >
          ...
          {ingredients && ingredients.length > 0 && (
            <div className="flex flex-col gap-2 pt-4 border-t border-black/5">
              ...
              <ul className="text-xs text-[#181813]/70 list-disc list-inside space-y-1">
        ...
        <div className="p-4 sm:p-6 bg-white border-t border-black/5 flex items-center justify-between gap-3">
          <button
            type="button"
            onClick={onClose}
            className="min-h-[44px] px-5 rounded-xl bg-black/5 hover:bg-black/10 text-xs font-semibold text-[#181813] transition-colors"
          >
            {t.menu.close}
          </button>
          ...
          {BUSINESS_DATA.hungerStationUrl && (
            <a
              ...
              className="min-h-[44px] flex-1 px-6 rounded-xl bg-[#122416] hover:bg-[#19321d] text-white text-xs font-bold flex items-center justify-center gap-2 active:scale-95 transition-all shadow-card"
            >
